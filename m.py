...
with tab1:
    ...
    with col_left:
        # ✅ 这是 Streamlit 最稳定的视频播放方式
        try:
            st.video("0ca6fb4753bc462ca6745c982f5ff55b.mp4")
        except:
            st.video("https://www.w3school.com.cn/i/movie.mp4")  # 备用在线视频
            st.warning("本地视频未找到，已加载演示视频")

    with col_right:
        st.markdown("### 🤖 AI 实时提示区")
        st.markdown("---")

        # ✅ 拖动滑块同步提示（最稳定、不卡顿、视频一定能打开）
        current_time = st.slider("拖动跟随视频进度", 0, 189, 0)
        st.progress(current_time / 189, text=f"当前进度：{current_time}s / 189s")
